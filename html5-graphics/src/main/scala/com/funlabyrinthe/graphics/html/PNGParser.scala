...
object PNGParser:
  val PNGBlobProperties: BlobPropertyBag = new { `type` = "image/png" }
  ...
  final class Frame:
    ...
    private[PNGParser] val dataParts = mutable.ArrayBuffer.empty[ByteBuffer]
    var imageData: Blob = uninitialized
  ...
  def parse(buffer: ByteBuffer): PNGInfo =
    ...
    if !apng.frames.isEmpty then
      ...
      for frame <- apng.frames do
        ...
        frame.imageData = new Blob(bb, PNGBlobProperties)
